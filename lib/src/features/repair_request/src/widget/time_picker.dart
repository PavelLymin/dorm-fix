  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<RequestFormBloc, RequestFormState>(
      listenWhen: (previous, current) =>
          ...
          previous.currentFormModel.endTime != current.currentFormModel.endTime,
      listener: (context, state) =>
          state.mapOrNull(initial: (_) => _initTime()),
      child: SizedBox(
        child: Row(
...
mixin _TimePickerStateMixin on State<TimePicker> {
  ...
  void _initTime() {
    _startTime.text = '${_workingDateTime.start.hour}:00';
    _endTime.text = '${_workingDateTime.end.hour}:00';
  }
  ...
  void _onSelectedStartTime(DateTime? value) {
    context.read<RequestFormBloc>().add(.update(startTime: value!));
    _endTimes = _endEntries(value.hour + 1);
  }

  void _onSelectedEndTime(DateTime? value) {
    context.read<RequestFormBloc>().add(.update(endTime: value!));
  }
  ...
  void _onChangeStart(DateTime? start, DateTime? end) {
    ...
    if (_workingDateTime.isEndBeforeOrSameStart(start, end)) {
      ...
      context.read<RequestFormBloc>().add(.update(endTime: end));
