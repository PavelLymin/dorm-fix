...
class RoomRouter {
  ...
  String _checkDormitoryId(Request request) {
    final dormitoryId = request.params['dormitoryId'];

    if (dormitoryId == null || dormitoryId.isEmpty) {
      throw BadRequestException(
        error: {
          'description': 'Missing or invalid dormitory id in request context.',
          'query_param': 'dormitoryId',
        },
      );
    }
  ...
  Future<Response> _searchRooms(Request request) async {
    ...
    final rooms = await _roomRepository.searchRooms(
      query: query,
      dormitoryId: int.parse(dormitoryId),
    );
