...
abstract interface class IRepairRequestFacade {
  ...
  Stream<List<RequestAggregate>> watchStudentRequests({required String uid});
}
...
class RepairRequestFacadeImpl implements IRepairRequestFacade {
  ...
  Stream<RequestAggregate> _watchRequestAggregate(int requestId) {
    ...
    final specialization = _specializationRepository.watchSpecialization(
      requestId: requestId,
    );
  ...
  @override
  Stream<List<RequestAggregate>> watchStudentRequests({required String uid}) {
    return _requestRepository.watchRequests(uid: uid).switchMap((requests) {
      if (requests.isEmpty) return Stream.empty();
  ...
  // @override
  // Stream<List<RequestAggregate>> watchStudentRequests({
  //   required String uid,
  // }) async* {
  //   yield* _database
  //       .customSelect(
  ...
  // @override
  // Stream<List<RequestAggregate>> watchMasterRequests({
  //   required int specId,
  //   required int dormId,
  // }) async* {
  //   yield* _database
  ...
  //         GROUP BY r.id, c.id
  //         ''',
  //         variables: [Variable<int>(specId), Variable<int>(dormId)],
  //         readsFrom: {
  //           _database.requests,
  //           _database.specializations,
  //           _database.problems,
